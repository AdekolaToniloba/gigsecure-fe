...
type UnassessedHeroProps = {
  description?: string;
  className?: string;
};

type UnassessedOverviewProps = {
...
const DEFAULT_DESCRIPTION =
  'Complete a few quick steps to personalize your experience and get the right protection for your needs.';

export function UnassessedHero({
...
export function UnassessedOverview({
  heroDescription,
  promptDescription,
  className,
}: UnassessedOverviewProps) {
  return (
    <section
      aria-label="Risk assessment onboarding"
      className={cn(
        'grid min-w-0 grid-cols-1 gap-5 lg:grid-cols-[minmax(0,3fr)_minmax(16rem,1fr)]',
        className
      )}
    >
      <UnassessedHero description={heroDescription} />
      <RiskAssessmentPrompt description={promptDescription} />
    </section>
  );
}
